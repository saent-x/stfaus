...
pub fn Settings() -> Element {
    let app_settings = use_resource(|| async move {
    ...
    let mut account_name: Signal<String> = use_signal(|| app_settings().user_name);
    ...
    let mut provider: Signal<String> = use_signal(|| app_settings().provider);
                ...
                div { class: "flex flex-row justify-center mt-10",
                    button {
                        ...
                        onclick: move |_| async move {
                            ...
                            let mut db = get_db().lock().await;
                            save_app_settings(&mut db, &app).expect("failed to save app settings");
